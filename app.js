...
// PLACEHOLDER: Replace these config keys with your own Firebase Console web app settings!
...
function listenToArticles() {
  db.collection('articles').orderBy('createdAt', 'desc').onSnapshot(async (snapshot) => {
    ...
    showToast("Database synchronization error: " + err.message);
  });
}
...
function openAuthModal(pane = 'signin') {
  ...
  if (pane === 'signin') {
    ...
    document.getElementById('author-role-group').classList.add('hidden');
  }
}
...
function setupEventListeners() {
  ...
  document.getElementById('signup-form').addEventListener('submit', (e) => {
    ...
    const role = document.getElementById('signup-role-type').value;
  ...
  document.getElementById('article-composer-form').addEventListener('submit', (e) => {
    ...
    // Save directly to Firestore articles collection
    db.collection('articles').doc(newArticleId).set(newArticle)
      .then(() => {
